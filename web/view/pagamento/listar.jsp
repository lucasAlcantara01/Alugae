<%@page import="dao.PagamentoDAO, dao.ClienteDAO, dao.CarroDAO, dao.LocacaoDAO"%>
<%@page import="model.Pagamento, model.Cliente, model.Carro, model.Locacao"%>
...
<html>
    ...
    <body>
        ...
        List<Pagamento> lista = pagamentoDAO.consultar_lista();
        %>
        <main>
            <div class="flex justify-between mb-4">
                <h2 class="text-3xl text-gray-900 font-semibold">Pagamentos</h2>
                <a
                    href="inserir/index.jsp"
                ...
                <table class="w-full text-sm text-left text-gray-700">
                    <thead class="text-gray-400">
                        <tr>
                            ...
                            <th class="px-4 py-3 font-normal">ID Locação</th>
                            <th class="px-4 py-3 font-normal">Nome do Cliente</th>
                            <th class="px-4 py-3 font-normal">Modelo e Placa do Carro</th>
                            <th class="px-4 py-3 font-normal">Tipo de Pagamento</th>
                            <th class="px-4 py-3 font-normal">Status do Pagamento</th>
                            <th class="px-4 py-3 font-normal">Data do Pagamento</th>
                            <th class="px-4 py-3 font-normal">Valor do Pagamento</th>
                    ...
                    <tbody class="bg-white">
                        <%
                            for (Pagamento pagamento : lista) {
                                Locacao locacaoObj = new Locacao();
                                locacaoObj.setId(pagamento.getIdLocacao());
                                Locacao locacao = locacaoDAO.consultar(locacaoObj);
                                ...
                                if (locacao != null) {
                                    ...
                                    clienteObj.setId(locacao.getIdCliente());
                                    cliente = clienteDAO.consultar(clienteObj);

                                    Carro carroObj = new Carro();
                                    carroObj.setId(locacao.getIdCarro());
                                    carro = carroDAO.consultar(carroObj);
                                }
                        %>
                        <tr class="border-b border-gray-100">
                            <td class="px-4 py-3 text-gray-500"><%= pagamento.getId() %></td>
                            <td class="px-4 py-3 text-gray-500"><%= pagamento.getIdLocacao() %></td>
                            <td class="px-4 py-3 font-semibold text-gray-900"><%= cliente != null ? cliente.getNome() : "N/A" %></td>
                            <td class="px-4 py-3 text-gray-400"><%= carro != null ? carro.getModelo() + " (" + carro.getPlaca() + ")" : "N/A" %></td>
                            <td class="px-4 py-3 text-gray-400"><%= pagamento.getTipoPagamento() %></td>
                            <td class="px-4 py-3 text-gray-400"><%= pagamento.getStatusPagamento() %></td>
                            <td class="px-4 py-3 text-gray-400"><%= pagamento.getDataPagamento() %></td>
                            <td class="px-4 py-3 text-gray-400">R$ <%= String.format("%.2f", pagamento.getValorTotal()) %></td>
                            <td class="px-4 py-3 text-center relative">
                                <div class="inline-block text-left">
                                    ...
                                    <div
                                        ...
                                        >
                                        <a
                                            href="./alterar/index.jsp?id=<%= pagamento.getId() %>"
                                        ...
                                        <a
                                            href="excluir/index.jsp?id=<%= pagamento.getId() %>"
                                            onClick="return confirm('Tem certeza que deseja excluir?')"
                                            class="group flex items-center gap-2 p-2 text-sm text-gray-600 hover:bg-red-100 hover:text-red-500 rounded-md transition-all"
                                            >
                                            Excluir
                                        </a>
                                    </div>
